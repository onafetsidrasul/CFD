...
% SITO DI RIFERIMENTO >> https://www.hpc.ntnu.no/display/hpc/OpenFOAM+-+Airfoil+Calculations %

% Foil geometry
c = 0.12;             % Geometric chord length
alpha = 0.0;          % Angle of attack (in radians)
NACA = [0 0 1 2];     % NACA 4-digit designation as a row vector;

% Mesh dimensions
scale = 1;            % Scaling factor
H = 0.15;             % *Half* height of channel
W = 0.001;            % *Half* depth of foil (y-direction)
D = 0.4;              % Length of downstream section

% Mesh resolution parameters
Ni = 300;             % Number of interpolation points along the foil
Nx = 270;             % Number of mesh cells along the foil
ND = 160;             % Number of cells in the downstream direction
NT = 300;             % Number of cells the transverse direction
NW = 1;               % Number of cells in the y-direction (along the foil axis)

% Expansion rates
ExpT = 240;           % Expansion rate in transverse direction
ExpD = 10;            % Expansion rate in the downstream direction
ExpArc = 30;          % Expansion rate along the inlet arc
...
SETTO TUTTI I GRADING A 1!

Poi domani faccio il probe, giuro